...
<section class="dashboard">
   ...
   <div class="box">
      <?php
         $total_completes = 0;
         $total_completescount=0;
         $select_completes = $conn->prepare("SELECT * FROM `orders` WHERE payment_status = ?");
         $select_completes->execute(['Completado']);
         while($fetch_completes = $select_completes->fetch(PDO::FETCH_ASSOC)){
            $total_completes += $fetch_completes['total_price'];
            $total_completescount=$total_completescount+1;
         }
      ?>
      <div style="display:flex;justify-content:space-between">
      <h3><span>S/.</span><?= $total_completes; ?><span></span></h3>
      <h3><span></span><?= $total_completescount; ?><span> pedidos</span></h3>
      </div>
   ...
   <div class="box">
      ...
      <a href="placed_orders.php" class="btn">Total de cotizaciones</a>
   </div>

   <div class="box">
      <?php
         $select_products = $conn->prepare("SELECT * FROM `productos_somuch`");
