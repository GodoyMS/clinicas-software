    ...
    <div class="min-h-screen flex flex-col flex-auto flex-shrink-0 antialiased bg-gray-100 dark:bg-gray-700 text-black dark:text-white">
      ...
      <div id="successUploadedImgModal"></div>
       ...
       <div class="flex justify-end pt-2">
                        <button class="text-blue-500 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-500 flex gap-2 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 focus:outline-none " type="button" data-drawer-target="drawer-right-example" data-drawer-show="drawer-right-example" data-drawer-placement="right" aria-controls="drawer-right-example">
                        ...
                        <p class="mb-6 text-sm text-gray-500 dark:text-gray-400 text-justify">Esta sección sirve para mantener una comunicación bilateral con tus pacientes. Aqui podrás comunicarte con cualquier paciente que este afiliado y con su cuenta activada. Esta sección es muy importante ya que recibiras notificaciones de solicitud de citas y solicitud de postergación de citas, pero no te precupes, podras saber si tienes notificaciones nuevas en el icono de mensajería en la parte superior. Tus pacientes tambíen podran escribirte por alguna inquietud, sugerencia o malestar, y podran recibir los mensajes que mandes por este canal como se muestra en la imágen</p>
    ...
    <div class="relative w-full h-full max-w-md md:h-auto">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="px-6 py-6 lg:px-8">
                <h3 class="mb-4 text-xl font-medium text-gray-900 dark:text-white">Nuevo mensaje</h3>
            </div>
                ...
                <input name="emailConsultorio" hidden value="<?=$fetch_clinica['emailConsultorio'];?>">
                      ...
                      <label for="name" class="text-gray-800 dark:text-white  text-sm font-bold leading-tight tracking-normal">Para</label>
                        <label for="underline_select" class="sr-only">Underline select</label>
                       
                        <select name="dni" id="underline_select" required class="mb-5 mt-2 text-gray-600 dark:bg-gray-900 dark:text-white dark:placeholder-gray-200 dark:border-gray-700 focus:outline-none focus:border focus:border-indigo-700 font-normal w-full h-10 flex items-center pl-3 text-sm border-gray-300 rounded border">
                    ...
                    <textarea id="message" name="mensaje" rows="4" required class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Escribe tu mensaje"></textarea>
                  
                
                    <button type="submit" name="enviarMensaje"class="w-full text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Enviar</button>
...
<h2 class="text-lg mt-8 mb-4 text-center text-gray-500 dark:text-gray-200    ">Mensajes recibidos</h2>
       ...
       if($select_mensajes->rowCount()>0){
        while($fetch_mensajes=$select_mensajes->fetch(PDO::FETCH_ASSOC)){
            ?>
                <div class="px-2">
                <div id="toast-notification" style="max-width:40rem" class="my-2 mx-auto w-full  p-4 text-gray-900 bg-white rounded-lg shadow-lg dark:bg-gray-800 dark:text-gray-300" role="alert">
                        <div class="flex justify-between items-center mb-3">
                            ...
                            if($fetch_mensajes['tipoMensaje']!=""){
                              ?>
                            <span class="bg-gray-200 p-2 rounded-lg text-xs dark:bg-gray-500 dark:text-white shadow-lg"> <?=$fetch_mensajes['tipoMensaje'];?></span>
                        ...
                        <div class="flex items-center">
                            ...
                            <div class="ml-3 text-sm font-normal">
                                <div class="text-sm font-semibold text-gray-900 dark:text-white"><?=$fetch_mensajes['nombrePaciente'];?></div>
                                <div class="text-sm font-normal "><?=$fetch_mensajes['mensaje'];?></div> 
                                <span class="text-xs font-medium text-blue-600 dark:text-blue-500"><?=$fetch_mensajes['emailPaciente'];?></span>   
                            </div>
                        </div>
                </div>
                </div>
        ...
        <div class="flex flex-col items-center">
          ...
          <span class="text-sm text-gray-700 dark:text-gray-400">
              Mostrando <span class="font-semibold text-gray-900 dark:text-white"><?=($page*5)-4;?></span> al <span class="font-semibold text-gray-900 dark:text-white"><?=($page*5)-5+$select_mensajes->rowCount();?></span> de <span class="font-semibold text-gray-900 dark:text-white"><?=$selectTotalMessages->rowCount();?></span> mensajes
          </span>
          <!-- Buttons -->
          <div class="inline-flex mt-2 gap-2 xs:mt-0">
              <?php
                  if($page <= 1 ){
                  ?>
                  <a  class="inline-flex  px-4 py-2 text-sm font-medium text-gray-200 bg-gray-500 rounded-l  dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400  ">
                  ...
                  ?>
                    <a  href="mensajes.php?page=<?=$page-1;?>"class="inline-flex px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-l hover:bg-blue-800 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
                    <svg aria-hidden="true" class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M7.707 14.707a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 1.414L5.414 9H17a1 1 0 110 2H5.414l2.293 2.293a1 1 0 010 1.414z" clip-rule="evenodd"></path></svg>
              ...
              <?php
                  ...
                  ?>
                  <a  class="inline-flex  px-4 py-2 text-sm font-medium text-white bg-gray-500 border-0 border-l border-gray-700 rounded-r   dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 ">
                      Siguiente
                  ...
                  ?>
              <a href="mensajes.php?page=<?=$page+1;?>" class="inline-flex  px-4 py-2 text-sm font-medium text-white bg-blue-600 border-0 border-l border-gray-700 rounded-r hover:bg-blue-800 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
                  Siguiente
                  <svg aria-hidden="true" class="w-5 h-5 ml-2" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M12.293 5.293a1 1 0 011.414 0l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-2.293-2.293a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
